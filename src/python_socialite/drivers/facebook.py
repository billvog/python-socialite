...
class FacebookProvider(AbstractDriver):
    ...
    def get_auth_url(self, state=None):
        url = f"https://www.facebook.com/{self.version}/dialog/oauth"
        return self.build_url(url, state)

    def get_token_url(self):
        return f"{self.graph_url}/{self.version}/oauth/access_token"
    ...
    def get_user_by_token(self, access_token):
        url = f"{self.graph_url}/{self.version}/me"
    ...
    def map_user_to_dict(self, raw_user):
        ...
        avatar_url = f"{self.graph_url}/{self.version}/{user_id}/picture/?type=large&width=1600"
